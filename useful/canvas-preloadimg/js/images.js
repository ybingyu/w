 ...
 */
function ImagesControl(index){
    this.init(index);
}
ImagesControl.prototype.init = function(index){
    var _this = this;
    ...
    this.preLoadImgs(this.data.aSrc);
}
/**
 * 绘制
 */
ImagesControl.prototype.drawProcess = function () {
    ...
    this.ctx.clearRect(this.data.nProcessX-5,this.data.nProcessY-5,this.data.nProcessW+15,this.data.nProcessH+300);
 ...
 */
ImagesControl.prototype.preLoadImgs = function (arr) {
    var arrNewImgs=[];
    ...
    function ImgsLoadPost(){
        ...
        if(_this.data.nLoadedImgs == arr.length){
            ...
            document.getElementById("fullMain").style.display = "";
...
ImagesControl.prototype.progressText = function(ctx, x, y, width, height, radius, max) {
    ctx.save();
    ctx.fillStyle = '#fff';
    var text = "玩命加载中..." + Math.floor(width/max*100)+"%";
