...
jQuery.fn.pagination = function(maxentries, opts){
 opts = jQuery.extend({
  ...
  jump_outofrange_text:"请输入正确的页码",
  jump_null_text:"请输入正确的页码",
  ellipse_text:"...",
 ...
 return this.each(function() {
  ...
  function drawLinks() {
   ...
   // sum total
   if(opts.isSum){
    // panel.append("第"+(current_page+1)+"页 共"+numPages()+"页 ");
   }
   // Generate "First"-Link
   if (opts.first_text
   ...
   if(opts.isJump){
    // Generate Jump Input
    panel.append(jQuery("<input type='text' style='text-align:center;font-weight:bold' class='jump-index' title='请输入页码' size='1' />"));
    ...
    panel.append(jump);
    panel.find('.jump-index').bind('keyup', function(event){
        if (event.keyCode=="13"){
       var jumpinput = panel.find('.jump-index');
       index = jumpinput.val();
       if(index == null || index == ""){
